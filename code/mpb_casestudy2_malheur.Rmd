---
title: "Plots, Planes, & Pixels, MPB Case Study"
date: "`r Sys.Date()`"
author: "D. Perret"
output: 
  rmdformats::robobook:
...
knitr::kable(summary %>% 
               arrange(CURR_TPH*-1) %>% 
               select(COMMON_NAME, MORT_TPH, MORT_PERC, CHNG_PERC, CURR_TPH),
             digits=2)
```
...
rFIA::tpa(db = fia,
  ...
  # bind_rows(data.frame(MEASYEAR = c(2011:2019),
  #                      TPA = 0,
  #                      TPA_SE = 0)) %>% 
  ggplot(.,
         aes(x = MEASYEAR,
...
ggplot() +
  ...
  geom_sf(data=mpb.all %>% 
          ...
          aes(fill = LEGACY_TPA),
            # fill = "dodgerblue2",
          col = NA, alpha = 0.6) +
  geom_sf(data = in.mpb %>% sf::st_as_sf() %>% filter(MEASYEAR>2010),
          col = "black", alpha = 0.2) +
  geom_sf(data = in.mpb %>% sf::st_as_sf() %>% filter(PLT_CN%in%insect.plots,MEASYEAR>2010),
          col = "red") +
...
fia$PLOT %>% 
  ...
  filter(MEASYEAR %in% c(2011:2019)) %>% 
  group_by(MEASYEAR) %>% 
...
## Comparing FIA area disturbed to ADS severity classes

So Danny recently moved to a percentage-based class system for severity, but older data have a TPA-mortality estimate. I'll need to get in touch with him to get the cross-walk for the two systems.

In the figure below, I split LEGACY_TPA estimates up into low/med/high classes using TPA thresholds of \<2, 2-5, and \>5 -- this kind of lines up with data range, but I really need to touch base with Danny about what makes the most sense.

```{r}

mpb.all <- mpb.all %>% 
  mutate(severity.class = case_when(LEGACY_TPA <= 2 ~ "low",
                                    LEGACY_TPA > 2 & LEGACY_TPA <= 10 ~ "med",
                                    LEGACY_TPA > 10 ~ "high"))

mpb.all %>% 
  sf::st_drop_geometry() %>% 
  group_by(SURVEY_YEA,severity.class) %>% 
  summarise(area = sum(ACRES)) %>% 
  ggplot(.,aes(x = SURVEY_YEA,
               y = area,
               col = severity.class)) +
...
## Matching FIA condition class curve with ADS severity curve
...
mpb.all %>% 
  ...
  group_by(SURVEY_YEA,severity.class) %>% 
  summarise(area = sum(ACRES)) %>% 
  filter(severity.class %in% c("med","high")) %>% 
  ggplot(.,aes(x = SURVEY_YEA,
               y = area,
               col = severity.class)) +
  geom_line(lwd=2) +
  geom_line(data = cond.area,
            aes(x = MEASYEAR, 
                y = area),
            col = "black",
            lwd=2) +
...
-   dig into damage codes -\> or break by FORTYPCD
    ...
    - only 89 lodgepole/ponderosa have bark beetle damage codes, over entire time period

-   MORTYR for finding outbreak upswing; also %dead\

-   break out ADS curve with mort severity

    - I did this but need to confirm the TPA mortality -- severity crosswalk with Danny so that I can make good severity classes.
...
-   calculate ADS annual "new area" (i.e., symmetric difference) to see how well that matches FIA estimate (gets at Danny's concern about recounting old damage)
    ...
    - this is kind of difficult to actually get at -- how do we know it's recounting old damage versus ongoing damage within a stand? 
    
    - how does this issue manifest in FIA data?
