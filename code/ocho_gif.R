...
library(ggmap)
ext <- sf::st_bbox(aoi %>% sf::st_as_sf() %>%  sf::st_transform(crs=CRS(old.proj)))
names(ext) <- c("left","bottom","right","top")

#m.all <- sf::read_sf("data/ADS_data/shapefiles/MPB2_malh_2011_2019.shp")

register_stadiamaps("d0a45b35-73bc-47d3-a8c7-8a0be7f39dc6")
ocho.map <- get_stadiamap(bbox = ext, mapytpe = "stamen_terrain_labels",zoom = 10)
...
ggmap(ocho.map) +
  geom_sf(data = aoi %>% 
          ...
          col = "black",
          lwd=1.5) +
  geom_spatraster(data = ocho.pred) +
  facet_wrap(~lyr)+
